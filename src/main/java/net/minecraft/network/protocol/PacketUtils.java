        ...
        if (!pExecutor.isSameThread()) {
            pExecutor.executeIfPossible(new PacketRunnable(pPacket, () -> {
                ...
                if (pProcessor.shouldHandleMessage(pPacket)) {
                    try {
                        // Arcane mixin port: SyncReceive can cancel packets on the main-thread dispatch path.
                        PacketEvent packetEvent = new PacketEvent(pPacket, PacketEvent.PacketType.SyncReceive);

...
public abstract class VoxelShape {
    ...
    protected VoxelShape(DiscreteVoxelShape pShape) {
        this.shape = pShape;
    }
    ...
    public VoxelShape move(Vec3 pOffset) {
        return this.move(pOffset.x, pOffset.y, pOffset.z);
    }
    ...
    public VoxelShape move(double pXOffset, double pYOffset, double pZOffset) {
        return (VoxelShape)(this.isEmpty()
            ...
            : new ArrayVoxelShape(
                this.shape,
                new OffsetDoubleList(this.getCoords(Direction.Axis.X), pXOffset),
                new OffsetDoubleList(this.getCoords(Direction.Axis.Y), pYOffset),
                new OffsetDoubleList(this.getCoords(Direction.Axis.Z), pZOffset)
            ));
    }
    ...
    public void forAllEdges(Shapes.DoubleLineConsumer pAction) {
        this.shape
            .forAllEdges(
                (p_83228_, p_83229_, p_83230_, p_83231_, p_83232_, p_83233_) -> pAction.consume(
    ...
    public void forAllBoxes(Shapes.DoubleLineConsumer pAction) {
        ...
        this.shape
            .forAllBoxes(
                (p_83239_, p_83240_, p_83241_, p_83242_, p_83243_, p_83244_) -> pAction.consume(
    ...
    protected int findIndex(Direction.Axis pAxis, double pPosition) {
        return Mth.binarySearch(0, this.shape.getSize(pAxis) + 1, p_166066_ -> pPosition < this.get(pAxis, p_166066_)) - 1;
    }
    ...
    public BlockHitResult clip(Vec3 pStartVec, Vec3 pEndVec, BlockPos pPos) {
        if (this.isEmpty()) {
            ...
            if (vec3.lengthSqr() < 1.0E-7) {
                ...
                Vec3 vec31 = pStartVec.add(vec3.scale(0.001));
                return this.shape
                        .isFullWide(
                            this.findIndex(Direction.Axis.X, vec31.x - (double)pPos.getX()),
                            this.findIndex(Direction.Axis.Y, vec31.y - (double)pPos.getY()),
                            this.findIndex(Direction.Axis.Z, vec31.z - (double)pPos.getZ())
                        )
                    ? new BlockHitResult(vec31, Direction.getApproximateNearest(vec3.x, vec3.y, vec3.z).getOpposite(), pPos, true)
                    : AABB.clip(this.toAabbs(), pStartVec, pEndVec, pPos);
            }
        }
    }

    // Arcane mixin port: Yarn name for official bounds().
    public AABB getBoundingBox() {
        return this.bounds();
    }

    // Arcane mixin port: Yarn calls this raycast(); official names it clip().
    public BlockHitResult raycast(Vec3 pStartVec, Vec3 pEndVec, BlockPos pPos) {
        return this.clip(pStartVec, pEndVec, pPos);
    }
    ...
    private boolean isCubeLikeAlong(Direction.Axis pAxis) {
        DoubleList doublelist = this.getCoords(pAxis);
    ...
    protected double collideX(AxisCycle pMovementAxis, AABB pCollisionBox, double pDesiredOffset) {
        if (this.isEmpty()) {
        ...
        } else if (Math.abs(pDesiredOffset) < 1.0E-7) {
            return 0.0;
        } else {
            ...
            int k = Math.max(0, this.findIndex(direction$axis1, pCollisionBox.min(direction$axis1) + 1.0E-7));
            int l = Math.min(this.shape.getSize(direction$axis1), this.findIndex(direction$axis1, pCollisionBox.max(direction$axis1) - 1.0E-7) + 1);
            int i1 = Math.max(0, this.findIndex(direction$axis2, pCollisionBox.min(direction$axis2) + 1.0E-7));
            int j1 = Math.min(this.shape.getSize(direction$axis2), this.findIndex(direction$axis2, pCollisionBox.max(direction$axis2) - 1.0E-7) + 1);
            int k1 = this.shape.getSize(direction$axis);
            if (pDesiredOffset > 0.0) {

...
public class Gui {
    ...
    private void renderCameraOverlays(GuiGraphics pGuiGraphics, DeltaTracker pDeltaTracker) {
            ...
            if (this.minecraft.player.isScoping()) {
                ...
                for (EquipmentSlot equipmentslot : EquipmentSlot.values()) {
                    ...
                    if (equippable != null && equippable.slot() == equipmentslot && equippable.cameraOverlay().isPresent()) {
                        ResourceLocation resourcelocation = equippable.cameraOverlay().get();
                        // Arcane mixin port: AntiBlind can cancel the pumpkin blur overlay before vanilla draws it.
                        AntiBlind antiBlind = Client.INSTANCE.getModuleManager().getModule(AntiBlind.class);
                        if (antiBlind != null && antiBlind.isState() && !AntiBlind.pumpkin.get() && "misc/pumpkinblur".equals(resourcelocation.getPath())) {
                            continue;
                        }

                        this.renderTextureOverlay(pGuiGraphics, resourcelocation.withPath(pCodec30_ -> "textures/" + pCodec30_ + ".png"), 1.0F);
                    }
                }
            }
        }
        ...
        if (f1 > 0.0F) {
            if (!this.minecraft.player.hasEffect(MobEffects.CONFUSION)) {
                ...
                if (f2 < 1.0F) {
                    float f3 = f1 * (1.0F - f2);
                    // Arcane mixin port: AntiBlind's nausea toggle controls whether the confusion overlay is rendered.
                    AntiBlind antiBlind = Client.INSTANCE.getModuleManager().getModule(AntiBlind.class);
    ...
    private void renderScoreboardSidebar(GuiGraphics pGuiGraphics, DeltaTracker pDeltaTracker) {
        ...
        if (objective1 != null) {
            // Arcane mixin port: custom Scoreboard module consumes the sidebar objective and renders it from its widget pass.
            cn.lazymoon.features.module.impl.render.Scoreboard arcaneScoreboard = Client.INSTANCE.getModuleManager()
                .getModule(cn.lazymoon.features.module.impl.render.Scoreboard.class);
            if (arcaneScoreboard != null && arcaneScoreboard.isState()) {
                arcaneScoreboard.setObjective(objective1);
                return;
            }
    ...
    private void renderEffects(GuiGraphics pGuiGraphics, DeltaTracker pDeltaTracker) {
        // Arcane mixin port: InterFace.hidePotion cancels the vanilla status-effect HUD.
    ...
    public void renderSelectedItemName(GuiGraphics graphicsIn, int yShift) {
        // Arcane mixin port: fire the Render2DEvent at the selected item tooltip hook.
        Render2DEvent event = new Render2DEvent(graphicsIn, 1.0F);
        event.getContext().pose().pushPose();
        Client.INSTANCE.getEventManager().call(event);
        event.getContext().pose().popPose();
    ...
    private void displayScoreboardSidebar(GuiGraphics pGuiGraphics, Objective pObjective) {
        ...
        Gui.DisplayEntry[] agui$1displayentry = scoreboard.listPlayerScores(pObjective)
            .stream()
            ...
            .map(entry2In -> {
                ...
                return new Gui.DisplayEntry(component2, component3, k3);
            })
        ...
        for (Gui.DisplayEntry gui$1displayentry : agui$1displayentry) {
        ...
        for (int i2 = 0; i2 < k2; i2++) {
            Gui.DisplayEntry gui$1displayentry1 = agui$1displayentry[i2];
            int j2 = i3 - (k2 - i2) * 9;
